...
test("Get TimeTable for Class ID", async () => {
  ...
  await webuntisApi.setCurrentSchoolyear(schoolYear);
...
test("Get All Lessons for School Year with specific classes", async () => {
  const year = await webuntisApi.getSchoolYearByName("2025/2026");
  const lessons = await webuntisApi.getAllLessonsForSchoolYear(
    year,
    "IT-Security",
    ["ITS-3"],
  );
